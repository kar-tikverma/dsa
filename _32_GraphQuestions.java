...
public class _32_GraphQuestions {
    ...
    public static int orangesRotting_2(int[][] grid) {
        ...
        for (int i = 0; i < grid.length; i++) {
            for (int j = 0; j < grid[0].length; j++) {
                if (grid[i][j] == 1)
                    return -1;
                if (grid[i][j] - 2 > minInfectingTime)
                    minInfectingTime = grid[i][j] - 2;
            }
        }
    ...
    private static int bfs(List<String> candidates, HashMap<String, Boolean> vis,
            HashMap<String, HashSet<String>> graph, String target, int length) {
        length++;
        if (candidates.size() == 0) {
            return 0;
        }
        HashSet<String> new_candidates = new HashSet<String>();
    ...
    public static int ladderLength_6(String beginWord, String endWord, List<String> wordList) {
        ...
        HashSet<String> forwardSet = new HashSet<String>();
        HashSet<String> backwardSet = new HashSet<String>();
    ...
    private static int search(Set<String> beginSet, Set<String> endSet, Set<String> dict, boolean isForward, int cnt) {
        ...
        return nextSet.size() > endSet.size() ? search(endSet, nextSet, dict, false, cnt)
    ...
    public static int findCheapestPrice_1(int n, int[][] flights, int src, int dest, int K) {
        ...
        while (!q.isEmpty()) {
            ...
            if (curr.stopNumber > K)
                break;
            if (graph.get(curr.vertex) == null)
                break;
