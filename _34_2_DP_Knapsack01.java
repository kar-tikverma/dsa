...
public class _34_2_DP_Knapsack01 {
    public static int knapsack01_recursion (int[] val, int[] wt, int W) {
        return knapsack01_recursion_Util(val.length, val, wt, W);
    }
    private static int knapsack01_recursion_Util (int n, int[] val, int[] wt, int W) {
        if (W == 0 || n == 0) {
            return 0;
        }
        ...
        int res1 = 0;
        if (wt[n-1] <= W) {
            res1 = val[n] + knapsack01_recursion_Util(n - 1, val, wt, W - wt[n]);
        }

        // exclude
        int res2 = knapsack01_recursion_Util(n - 1, val, wt, W);
